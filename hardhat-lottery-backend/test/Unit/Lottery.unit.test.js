...
describe("Lottery contract tests",()=>{
    ...
    describe("Enter lottery",()=>{
        ...
        it("Does not allow other player to enter the lottery while it is calculating",async ()=>{
            ...
            const aa=await lottery.performUpkeep("0x") //IMP Send 0x as argument for calldata
                                                       //For memeory type send 0x0 as args
             console.log(aa);
    ...
    describe("Test for checkUpkeep",()=>{
        it("Should return false if lottery is in calculating state",async ()=>{
            ...
            const { upkeepNeeded } = await lottery.checkUpkeep.staticCall("0x")//Using
            //staticall so that we can get what the function returns
